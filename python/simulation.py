...
class TemperatureSimulator:
    ...
    def simulate(self, step: int) -> tuple[float, bool]:
        # --- 1. Variable cycle (non-repeating) ---
        # Slightly vary the "day length"
        period = 50 + random.uniform(-5, 5)
        self._phase += (2 * np.pi) / period

        # Add a second harmonic to break symmetry
        daily_cycle = (
            AMPLITUDE * np.sin(self._phase) +
            0.3 * AMPLITUDE * np.sin(2 * self._phase + 1.5)
        )

        # --- 2. Slow baseline drift ---
        self._baseline_drift += random.uniform(-0.02, 0.02)

        # --- 3. Correlated noise (random walk) ---
        self._noise_state += random.uniform(-NOISE_LEVEL, NOISE_LEVEL)
        self._noise_state *= 0.9  # dampen runaway drift

        temp = BASE_TEMP + daily_cycle + self._baseline_drift + self._noise_state
        ...
        if random.random() < ANOMALY_PROB:
            ...
            temp += spike * (0.5 + 0.5 * np.sin(self._phase))
            is_anomaly = True

        return round(temp, 2), is_anomaly


_default_simulator = TemperatureSimulator()


def simulate_temperature(step: int) -> tuple[float, bool]:
    return _default_simulator.simulate(step)